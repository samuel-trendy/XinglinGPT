...
from tcm_categories import name_en2zh
choices = ["A", "B", "C", "D", "E"]
...
def run_eval(model, tokenizer, eval, args):
    ...
    for subject in subjects:
        ...
        if os.path.exists(out_file):  # If result file exist, skip this subject
            continue
        dev_df = pd.read_csv(os.path.join(args.data_dir, "dev", subject + ".csv"), header=0)
        test_df = pd.read_csv(os.path.join(args.data_dir, "test", subject + ".csv"), header=0)
        #print(args.data_dir)
        #print(dev_df.shape)
...
def get_results(result_dir=''):
    ...
    for subject in name_en2zh.keys():
        try:
            ...
            continue
        df = pd.read_csv(file, names=['id','question','A','B','C','D','E','answer','response'], index_col=0)
    ...
    print(f"{'Overall'}:{avg_all_acc:.3f}")
